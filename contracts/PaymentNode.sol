...
contract BevorPaymentNode is OwnerIsCreator {
    ...
    // Mapping to track admin contracts that have privileged access
    mapping(address => bool) private s_admins;

    /// @notice Event emitted when an admin contract is added or removed
    ...
    /// @notice Adds or removes an admin contract
    /// @param _admin The address of the admin contract
    /// @param _status True to add admin, false to remove
    function setAdmin(address _admin, bool _status) external onlyOwner {
        s_admins[_admin] = _status;
        emit AdminStatusChanged(_admin, _status);
    }

    /// @dev Modifier that allows owner or admin contracts to call a function
    modifier onlyOwnerOrAdmin() {
        require(owner() == msg.sender || s_admins[msg.sender], "Caller is not owner or admin");
        _;
    }
